...
bool current_ext_spk_pa_state = false;//LINE<><20140917><ext-amp>wangyanhui
...
//BEGIN<><20140917><ext-amp>wangyanhui
...
{ 
	 ...
	 switch (ucontrol->value.integer.value[0]) { 
	 	...
	 	case 1: 
	 		if(gpio_is_valid(ext_spk_pa_gpio)){ 
				//BEGIN<><20141023><AMP request timing>wangyanhui
				...
				//udelay(2);
				//gpio_direction_output(ext_spk_pa_gpio, 1);
				//END<><20141023><AMP request timing>wangyanhui
...
//END<><20140917><ext-amp>wangyanhui
...
static const struct snd_kcontrol_new msm8x16_wcd_snd_controls[] = {
	...
	SOC_ENUM_EXT("Speaker Ext", msm8x16_wcd_ext_spk_ctl_enum[0], 
		msm8x16_wcd_ext_spk_get, msm8x16_wcd_ext_spk_set),//LINE<><20140911><ext-amp>wangyanhui
	...
	//SOC_SINGLE("MICBIAS CAPLESS Switch",
		   //MSM8X16_WCD_A_ANALOG_MICB_1_EN, 6, 1, 0),  //LINE<BUG><FFBAKK-107><>wangyanhui
#else
    //2014/11/03 tyj. bit6=0:EXT_BYP_CAP 1:NO_EXT_BYP_CAP.
    //ext cap is used for L5510 so bit6 should be set to 0.
	SOC_SINGLE("MICBIAS CAPLESS Switch",
	   MSM8X16_WCD_A_ANALOG_MICB_1_EN, 6, 0, 0),
#endif
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
        ...
        //LINE<JIRA_ID><DATE20141219><headset button wrong event in call>zenghaihui
        #if 0
		snd_soc_update_bits(codec,
				MSM8X16_WCD_A_ANALOG_MICB_1_EN, 0x05, 0x04);
        #else
	pr_info("%s :%d] zenghh set micbias 0x44\n", __func__, __LINE__);
        snd_soc_update_bits(codec,
