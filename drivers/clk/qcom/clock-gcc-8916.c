...
#if defined(CONFIG_LEDS_GPIO_CLK)

#define LEDS_GPIO_CLK_DEBUG
#ifdef DBG
#undef DBG
#endif
#ifdef LEDS_GPIO_CLK_DEBUG
#define DBG(fmt, args...) pr_err("[leds-gpio-clk] " fmt, ##args)
#else
#define DBG(fmt, args...) pr_debug(fmt, ##args)
#endif

#define DEFAULT_GP_CLK 12000

#endif /* CONFIG_LEDS_GPIO_CLK */
...
#if defined(CONFIG_LEDS_GPIO_CLK)	
	F( 10240,	   	      xo,  7.5,	  1,	250),
	F( 12000,	   	      xo,  16,	  1,	100),	
	F( 102400,	      xo,  7.5,	  1,	25),	
	F(  19200000,	      xo,   1,	  0,	0),
#endif	
	F( 100000000,	   gpll0,   8,	  0,	0),
	F( 200000000,	   gpll0,   4,	  0,	0),
...
static struct rcg_clk camss_gp0_clk_src = {
	...
	.c = {
		...
		.ops = &clk_ops_rcg_mnd,
#if defined(CONFIG_LEDS_GPIO_CLK)
		VDD_DIG_FMAX_MAP2(LOW, 10240, NOMINAL, 200000000),
#else
		VDD_DIG_FMAX_MAP2(LOW, 100000000, NOMINAL, 200000000),
#endif
...
static struct clk_freq_tbl ftbl_gcc_mdss_esc0_clk[] = {
	F(  12800000,	      xo,   1.5,	  0,	0),		//modfiy esc clock and div for LPX timming 20150325
	F_END
};
...
#if defined(CONFIG_LEDS_GPIO_CLK)
#define GP_CLK_GPIO 31

#define GCC_MM_GP0_CMD_RCGR 0x54000
#define GCC_MM_GP0_D 0x54010

static unsigned int *get_gcc_mm_gp0_d_addr(void)
{
	return (unsigned int *)(((unsigned char *)virt_bases[GCC_BASE]) + GCC_MM_GP0_D);
}

static unsigned int *get_gcc_mm_gp0_cmd_addr(void)
{
	return (unsigned int *)(((unsigned char *)virt_bases[GCC_BASE]) + GCC_MM_GP0_CMD_RCGR); 
}

#if 0
#define GCC_MM_GP0_CFG_RCGR 0x54004
#define GCC_MM_GP0_N_RCGR 0x5400C
unsigned int *get_gcc_mm_gp0_cfg_addr(void)
{
	return (unsigned int *)(((unsigned char *)virt_bases[GCC_BASE]) + GCC_MM_GP0_CFG_RCGR); 
}
unsigned int *get_gcc_mm_gp0_n_addr(void)
{
	return (unsigned int *)(((unsigned char *)virt_bases[GCC_BASE]) + GCC_MM_GP0_N_RCGR); 
}
#endif

extern int extern_set_gpio(int gpio_pin);

static void gcc_camss_gp0_clk_control(bool on)
{
	bool sum_ret = true;
	int ret = 0;
	...
	if(on){
		if(son){
			DBG("%s : gp_clk already on.\n", __func__);
			return ;
		}

		ret = clk_set_rate(&camss_gp0_clk_src.c, DEFAULT_GP_CLK);
		if(ret < 0){
			sum_ret = false;
			pr_err("%s : clk_set_rate for camss_gp0_clk_src, ret=%d\n", __func__, ret);
		}
		
		ret = clk_prepare_enable(&camss_gp0_clk_src.c);
		if(ret < 0){
			sum_ret = false;
			pr_err("%s : clk_prepare_enable for camss_gp0_clk_src, ret=%d\n", __func__, ret);
		}

		ret = extern_set_gpio(GP_CLK_GPIO);
		DBG("%s : extern_set_gpio return =%d\n", __func__, ret);
		
		ret = clk_set_rate(&gcc_camss_gp0_clk.c, DEFAULT_GP_CLK);
		if(ret < 0){
			sum_ret = false;
			pr_err("%s : clk_set_rate for gcc_camss_gp0_clk, ret=%d\n", __func__, ret);
		}
		
		ret = clk_prepare_enable(&gcc_camss_gp0_clk.c);
		if(ret < 0){
			sum_ret = false;
			pr_err("%s : clk_prepare_enable for gcc_camss_gp0_clk, ret=%d\n", __func__, ret);
		}
		...
		if(!son){
			DBG("%s : gp_clk already off.\n", __func__);
			return ;
		}
		son = false;
		clk_disable_unprepare(&gcc_camss_gp0_clk.c);
		clk_disable_unprepare(&camss_gp0_clk_src.c);
	}
}

int gpio_clk_led_set(int level)
{
	int ret = 0;
	static unsigned int spre_d_value = 0;
	unsigned int * gcc_mm_gp0_d_address = NULL;
	unsigned int * gcc_mm_gp0_cmd_rcgr = NULL;

	DBG("%s : level=%d\n", __func__, level);
	
	level = (level*100)/255;	//<LINE><FFBAKK-904><20141114>fix bug of backlight;xiongdajun
	if(level <= 0){
		level = 1;
	}
	if(level >= 100){
	...
	if(spre_d_value == ((~( 2 * level )) & 0xff)){
		return 0;
	}
	spre_d_value = (~( 2 * level )) & 0xff;

	if(level <= 1){
		gcc_mm_gp0_d_address = get_gcc_mm_gp0_d_addr();
		gcc_mm_gp0_cmd_rcgr = get_gcc_mm_gp0_cmd_addr(); 
		gcc_mm_gp0_d_address[0] = spre_d_value;
		mb();
		gcc_mm_gp0_cmd_rcgr[0] = 0x3;
		mb();
		gcc_camss_gp0_clk_control(0);
	}else{
		gcc_camss_gp0_clk_control(1);
		gcc_mm_gp0_d_address = get_gcc_mm_gp0_d_addr();
		gcc_mm_gp0_cmd_rcgr = get_gcc_mm_gp0_cmd_addr(); 
		gcc_mm_gp0_d_address[0] = spre_d_value;
		DBG("%s : D=%d; level=%d\n", __func__, spre_d_value, level);
		mb();
		gcc_mm_gp0_cmd_rcgr[0] = 0x3;
		mb();
	}
