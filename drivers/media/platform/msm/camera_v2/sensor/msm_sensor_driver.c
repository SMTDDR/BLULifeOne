...
//Add By YangRujin Start : store camera info 
...
//<Begin><FFBAKK-802><20141029>Add 5M camera infor xiongdajun
extern int store_sub_camera_info(const char *const str);
static char sub_camera_and_eeprom[DEV_INFO_LEN];
//<End><FFBAKK-802><20141029>Add 5M camera infor xiongdajun
//extern int store_main_camera_eeprom_info(const char *const str);
//extern int store_sub_camera_eeprom_info(const char *const str);
#endif
//Add By YangRujin End.
...
//Add By YangRujin Start : store camera info 
#ifdef CONFIG_DEV_INFO
	//<BEGIN><FFBAKK-802><20141029>Add 5M camera infor xiongdajun
	if(!strncmp(slave_info->sensor_name, "ov5648_q5v22e", sizeof("ov5648_q5v22e")))
	{
		if(!strncmp(s_ctrl->sensordata->eeprom_name, "daling_p5v23c", sizeof("daling_p5v23c"))){
		//store_sub_camera_eeprom_info("DaLing");

			rc = snprintf(sub_camera_and_eeprom, DEV_INFO_LEN, "%s_%s", 
				slave_info->sensor_name, "DaLing");		
		}else if(!strncmp(s_ctrl->sensordata->eeprom_name, "cat24c16", sizeof("cat24c16"))){
		//store_main_camera_eeprom_info("Liteon");//GuangBao

			rc = snprintf(sub_camera_and_eeprom, DEV_INFO_LEN, "%s_%s", 
				slave_info->sensor_name, "Liteon");		
		}else  if(!strncmp(s_ctrl->sensordata->eeprom_name, "primax_imx179", sizeof("primax_imx179"))){
		//store_main_camera_eeprom_info("Sunny");

			rc = snprintf(sub_camera_and_eeprom, DEV_INFO_LEN, "%s_%s", 
				slave_info->sensor_name, "Sunny");		
		}
		if(rc < 0){
	...
	//Begin <REQ><20150105><Add information of camera for L9010>;xiongdajun
	#ifdef CONFIG_PROJECT_L9010
	else if(!strncmp(slave_info->sensor_name, "ov5648_q5v22e_l9010", sizeof("ov5648_q5v22e_l9010")))
	{
		if(!strncmp(s_ctrl->sensordata->eeprom_name, "daling_p5v23c", sizeof("daling_p5v23c"))){
		//store_sub_camera_eeprom_info("DaLing");
	...
	#endif
	//End <REQ><20150105><Add information of camera for L9010>;xiongdajun
	else
	{
	//<END><FFBAKK-802><20141029>Add 5M camera infor xiongdajun
		if(!strncmp(s_ctrl->sensordata->eeprom_name, "daling_p5v23c", sizeof("daling_p5v23c"))){
			...
			//store_main_camera_eeprom_info("Liteon");//GuangBao
            if(!strncmp(slave_info->sensor_name, "imx135_s", sizeof("imx135_s")))
                rc = snprintf(main_camera_and_eeprom, DEV_INFO_LEN, "%s_%s", "imx135", "Liteon");            
            else
			rc = snprintf(main_camera_and_eeprom, DEV_INFO_LEN, "%s_%s", 
					slave_info->sensor_name, "Liteon");		
		}else  if(!strncmp(s_ctrl->sensordata->eeprom_name, "primax_imx179", sizeof("primax_imx179"))){
		...
		//Begin <REQ><JABAL-1500><20150528>add 3M2 camera;xiongdajun
		else  if(!strncmp(s_ctrl->sensordata->eeprom_name, "sunny_f13s01k", sizeof("sunny_f13s01k"))){
			//store_main_camera_eeprom_info("Sunny");

			rc = snprintf(main_camera_and_eeprom, DEV_INFO_LEN, "%s_%s", 
					slave_info->sensor_name, "Sunny");		
		}
		//End <REQ><JABAL-1500><20150528>add 3M2 camera;xiongdajun
		//Begin <REQ><20150105><Add information of camera for L9010>;xiongdajun
		#ifdef CONFIG_PROJECT_L9010
		else  if(!strncmp(s_ctrl->sensordata->eeprom_name, "sunny_p8v12b", sizeof("sunny_p8v12b"))){
			//store_main_camera_eeprom_info("Sunny");

			rc = snprintf(main_camera_and_eeprom, DEV_INFO_LEN, "%s_%s", 
					slave_info->sensor_name, "Sunny");		
		}else  if(!strncmp(s_ctrl->sensordata->eeprom_name, "primax_imx219", sizeof("primax_imx219"))){
			//store_main_camera_eeprom_info("Sunny");
